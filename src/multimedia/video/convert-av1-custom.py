...
def get_rotation(input_file: Path) -> int:
    try:
        result = subprocess.run(
            [
                "ffprobe",
...
def convert_video(
    ...
    try:
        ...
        if resolution > 0:
            vf_filters.append(
                f"scale='if(gt(a,1),-2,{resolution})':'if(gt(a,1),{resolution},-2)'"
            )
        ...
        if threads > 0:
            cmd.extend(["-svtav1-params", f"lp={threads}"])
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Output resolution height (e.g., 1080, 2160). Default: 0 (same as input)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of threads to use. Default: 0 (let ffmpeg decide)",
    )
    ...
    # --- MODE 1: SINGLE FILE ---
    if src_path.is_file():
        convert_video(src_path, dst_path, preset, crf, args.threads, args.resolution)
    ...
    # --- MODE 2: DIRECTORY (Recursive) ---
        ...
        for item in src_path.rglob("*"):
            ...
            if item.suffix.lower() in VIDEO_EXTS:
                ...
                convert_video(
                    item, out_file, preset, crf, args.threads, args.resolution
                )
